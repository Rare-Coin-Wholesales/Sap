...
namespace Sap.Aabw.Automation
{
	public partial class InvoiceUtil
	{
		...
		public async Task GetAllInvoices(SLConnection serviceLayer)
		{
			...
			var list = await Common.AabwServiceLayer.GetAllInvoicesAsync();
			...
			if (list == null || list.Count == 0) {
				...
				foreach (var v in list) {
					try {
						_invoiceService.Insert(Common._aabwMapper.ToSql(v));

						foreach (var line in v.DocumentLines) {
							try {
								_invoiceService.InsertDocumentLine(Common._aabwMapper.ToSql(line));
							}
