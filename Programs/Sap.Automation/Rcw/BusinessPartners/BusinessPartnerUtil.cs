		...
		public async Task GetAllBusinessPartners(SLConnection serviceLayer)
		{
			...
			if (list == null || list.Count == 0)
				...
				foreach (var v in list) {
					try {
						_businessPartnerService.Insert(Common._rcwMapper.ToSql(v));

						#region Insert BPAddress
						foreach (var line in v.BPAddresses) {
							try {
								_businessPartnerService.Insert(Common._rcwMapper.ToSql(line));
							}
						...
						//_businessPartnerService.Insert(Common._rcwMapper.ToSql(v.BPIntrastatExtension));
						#endregion
