...
namespace Sap.Automation
{
	public partial class JournalEntryUtil
	{
		...
		public void GetAllJournalEntrys()
		{
			Common.logger.Trace("Begin method GetAllJournalEntrys().");
			...
			if (list == null || list.Count == 0) {
				...
				foreach (var v in list) {
					try {
						_journalEntryService.Insert(Common._mapper.ToSql(v));

						foreach (var line in v.JournalEntryLines) {
							try {
								line.JdtNum = v.JdtNum;
								_journalEntryService.InsertJournalEntryLine(Common._mapper.ToSql(line));
							}
