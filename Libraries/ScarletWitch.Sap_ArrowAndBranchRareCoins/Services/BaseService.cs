...
namespace ScarletWitch.Sap_ArrowAndBranchRareCoins.Services
{
	/// <summary>
	/// Base service.
	/// </summary>
	public partial class BaseService
	{
		...
		protected Sap_ArrowAndBranchRareCoinsDb _dbContext;
		...
		public BaseService()
		{
			...
			ConnectionString = _encryptionUtil.Decrypt(CommonUtil.GetEnvironmentVariable("ScarletWitch.Sap_ArrowAndBranchRareCoins"));
			_dbContext = new Sap_ArrowAndBranchRareCoinsDb(ConnectionString);
		}
		...
		/// <summary>
		/// Starts SQL Agent job "Sap_ArrowAndBranchRareCoins || Triggered". The stored procedure that gets called depends on the Job ID being
		...
		/// </summary>
		public virtual void StartJob_Sap_ArrowAndBranchRareCoins_Triggered()
		{
			try {
				_dbContext.Database.ExecuteSqlCommand("[dbo].[StartJob_Sap_ArrowAndBranchRareCoins_Triggered]");
			}
		...
		protected virtual void TruncateTable(string table)
		{
			...
			_dbContext.Database.ExecuteSqlCommand($"TRUNCATE TABLE [Sap_ArrowAndBranchRareCoins].[dbo].[{table}]");
		}
	}
}