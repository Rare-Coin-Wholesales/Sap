	...
	public partial class AccountCategoryUtil
	{
		...
		public async Task GetAllAccountCategorys(SLConnection serviceLayer)
		{
			...
			foreach (var v in list) {
				try {
					_accountCategoryService.Insert(Common._aabwMapper.ToSql(v));
				}

				catch (Exception ex) {
					#region Log
					if (ex.InnerException == null)
						Common.nLog.Warn("{0}{2}Exception thrown running _accountCategoryService.Insert(Common._aabwMapper.ToSql(v)).{2}{1}{2}{2}", ex.Message, ex, Environment.NewLine);
					else
						throw;
					#endregion
				}
