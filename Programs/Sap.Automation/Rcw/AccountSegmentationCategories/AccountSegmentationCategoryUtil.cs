	...
	public partial class AccountSegmentationCategoryUtil
	{
		...
		public async Task GetAllAccountSegmentationCategorys(SLConnection serviceLayer)
		{
			...
			foreach (var v in list) {
				try {
					_accountSegmentationCategoryService.Insert(Common._rcwMapper.ToSql(v));
				}

				catch (Exception ex) {
					#region Log
					if (ex.InnerException == null)
						Common.nLog.Warn("{0}{2}Exception thrown running _accountSegmentationCategoryService.Insert(Common._rcwMapper.ToSql(v)).{2}{1}{2}{2}", ex.Message, ex, Environment.NewLine);
					else
						throw;
					#endregion
				}
