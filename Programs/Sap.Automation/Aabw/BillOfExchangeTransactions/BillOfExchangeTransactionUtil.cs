		...
		public async Task GetAllBillOfExchangeTransactions(SLConnection serviceLayer)
		{
			...
			if (list == null || list.Count == 0)
				...
				foreach (var v in list) {
					try {
						_billOfExchangeTransactionService.Insert(Common._aabwMapper.ToSql(v));
					}

					catch (Exception ex) {
						#region Log
						if (ex.InnerException == null)
							Common.nLog.Warn("{0}{2}Exception thrown running _billOfExchangeTransactionService.Insert(Common._aabwMapper.ToSql(v)).{2}{1}{2}{2}", ex.Message, ex, Environment.NewLine);
						else
							throw;
						#endregion
					}
