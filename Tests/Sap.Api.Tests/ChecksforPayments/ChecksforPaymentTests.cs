...
{
	public partial class ChecksforPaymentTests
	{
		...
		{
			AddTraceLogs();
			var all = await _serviceLayer.Request("ChecksforPayment").GetAllAsync<object>();
		}

		[Fact]
		public async Task Test_GetChecksforPaymentsByReferenceDateAsync()
		{
			AddTraceLogs();
			var testDate = DateTime.Today.AddDays(-14);
			//await _serviceLayer.GetChecksforPaymentsByReferenceDateAsync(AllDateThreshold, 4000);
		}
	}
}