	...
	public partial class AccountSegmentationUtil
	{
		...
		public async Task GetAllAccountSegmentations(SLConnection serviceLayer)
		{
			...
			foreach (var v in list) {
				try {
					_accountSegmentationService.Insert(Common._aabrcMapper.ToSql(v));
				}

				catch (Exception ex) {
					#region Log
					if (ex.InnerException == null)
						Common.nLog.Warn("{0}{2}Exception thrown running _accountSegmentationService.Insert(Common._aabrcMapper.ToSql(v)).{2}{1}{2}{2}", ex.Message, ex, Environment.NewLine);
					else
						throw;
					#endregion
				}
