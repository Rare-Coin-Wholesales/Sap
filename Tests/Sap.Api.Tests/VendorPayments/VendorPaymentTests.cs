...
{
	public partial class VendorPaymentTests
	{
		...
		{
			AddTraceLogs();
			var all = await _serviceLayer.Request("VendorPayments").GetAllAsync<object>();
		}

		[Fact]
		public async Task Test_GetVendorPaymentsByReferenceDateAsync()
		{
			AddTraceLogs();
			var testDate = DateTime.Today.AddDays(-31);
			await _serviceLayer.GetVendorPaymentsByDocDateAsync(testDate, 400);
		}
	}
}